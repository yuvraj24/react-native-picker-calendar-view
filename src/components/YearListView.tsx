...
const YearListView = ({
	...
	const flatRef = useRef<FlatList<MonthListItemProps>>(null);

	return (
		<View style={[styles.floatStyle, styles.parent]}>
			<TouchableOpacity
				style={[styles.floatStyle, styles.blurStyle]}
				onPress={() => toggleYearList && toggleYearList()}
			/>
			<FlatList
				onLayout={() =>
					flatRef.current?.scrollToIndex({ index: 10, animated: true })
				}
				ref={flatRef}
				...
				keyExtractor={(item: MonthListItemProps, index: number) =>
					`${index + item.value}`
				}
